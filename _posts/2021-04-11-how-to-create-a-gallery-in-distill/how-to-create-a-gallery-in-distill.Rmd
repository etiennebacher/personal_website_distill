...
[Lightgallery.js]() is a great Javascript library that allows you to build a gallery very simply. You will need images in full size and thumbnails, i.e a smaller version of the images (we will see how to automatically make them later in this post). 
...
Note that the names of the created thumbnails contain `thumb`. This is useful if you want to customize this function so that `list_png` only contains the full-size images and not the thumbnails that already exist.
...
We can now create a function to apply this type of code to all the images we have:
...
You need to add `fs::dir_copy("_gallery/img", "_site/_gallery/img")` in your GitHub Actions so that the images are found when the gallery is built. You also have to add `magick` and `httr` in the list of packages to install.
...
The purpose of the function below is to get a plot for a specific week in a specific year in the [repo containing my plots](https://github.com/etiennebacher/tidytuesday).
