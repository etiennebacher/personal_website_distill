...
I know that there are a bunch of packages that take performance very
seriously (for example `data.table` and `collapse`). I'm only showing some base R
...
`rowSums()`. First, we create a data frame full of `TRUE`/`FALSE`, and then we 
count by row the number of `TRUE`. If this number is equal to the number of columns,
then it means that the row only has `NA` or `""`.
