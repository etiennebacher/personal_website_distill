...
button, etc.) with some R code. This is very useful to perform some webscraping on pages that are *dynamically updated*, meaning that we can't simply use the URL to get the HTML we want. I won't explain the basics of webscraping here, there are plenty of articles on this already and the [vignettes on the `rvest` website](https://rvest.tidyverse.org/articles/rvest.html) are very good.


Something I mentioned very briefly in my [RSelenium slides](https://www.rselenium-teaching.etiennebacher.com) is that, in some cases, it is possible to avoid using RSelenium at all by performing the POST requests ourselves^[In fact, I completely buried this in the Appendix because RSelenium was already plenty to teach and I didn't want to add another layer of complexity with POST requests.]. However, a few days ago, another case of dynamic webscraping came to me and I thought that maybe that was a good opportunity to try doing this first. But before diving into this example, let's explain a bit more POST (and GET) requests and why using them directly could save a lot of time.


# POST and GET requests: what?

There are already a lot of resources on POST and GET requests, like [this W3schools page](https://www.w3schools.com/tags/ref_httpmethods.asp), so here I will only do a brief summary.
...
A GET request is used to obtain some data from the server and display it in another webpage. One important thing to note is that the query parameters (which describe what data we want to obtain) are displayed in the URL after a question mark `?`. That description might make some experts scream, but it is the simplest representation I have in mind and I think it corresponds more or less to what actually happens.
...
Basically, using RSelenium for dynamic webscraping boils down to go to the right page, fill the inputs so that we can perform a POST request (behind the scenes), and then scrape the data that the server provided. 

But what if we could avoid doing all these steps by performing the POST requests directly from R? That would be quicker and also would avoid all issues related to RSelenium. However, for this to work, we need to know the detailed request to send to the server.
...
The Spanish Office of Patents and Brands^[Oficina Española de Patentes y Marcas.] has a [nice website](http://historico.oepm.es/buscador.php) where you can get historical data about patents and brands from about mid-19th century to mid-20th century. You can type anything you want in the search bar (e.g a city name), specify the years range, and it provides a list of patents, their title, the exact date at which they were done, and some information on the person or company that deposited the patent.

Ideally, what we would like is to obtain all the information that is displayed when we click on the "+" button, and to do this for all records.
...
To see the requests and responses, we can use the tab "Network" in the developer console. Fill the search input (for example type "Madrid"), open the network tab and then click on the magnifying glass icon. You can see what we send to the server and what the server sends back:
...
You can see additional information in `add_headers()`. This information is available in the "Headers" tab when the click on the request. However, I don't really know why some of these headers are necessary and others are not. 
...
This is the HTML code from the "Mostrando xxx de xxx resultados encontrados" to the bottom of the page. We only want the table, so we can extract it with `rvest::html_table()`:
...
# remove cases where there were no attributes
...
There's still some cleaning to do but this post is quite dense so I'll stop here. The full code (POST + GET + cleaning) can be found on this repo, in the file "demo.R": [webscraping-patents-spain](https://github.com/etiennebacher/webscraping-patents-spain). Thanks for having read so far!