...
There are two main packages to do this: `httr` and `httr2`. The latter is just a couple of years old and has being developed with a lot of convenient built-in features (retries if a query failed, secure secrets, etc.^[I'm just listing things that are written on the [website of `httr2`](https://httr2.r-lib.org/), more details there.]).
...
**Note:** this website is in HTTP only, not HTTPS. Therefore, if you use Firefox with the mode HTTPS Everywhere enabled, you need to put http://historico.oepm.es in the list of exceptions (see [here](https://support.mozilla.org/en-US/kb/https-only-prefs)). 
