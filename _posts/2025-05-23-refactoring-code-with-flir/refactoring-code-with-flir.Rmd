...
We can then reuse those metavariables when we define the `fix` and `message`, but we'll need to use double tilde instead of `$` to use them:
...
The only thing left to do is to list in the `flir/config.yml` file so that it is taken into account by `flir::fix_*()` functions:
...
Alternatively, we can leave it out of the config file and specify it by hand, such as `flir::fix("my_file.R", linters = "superseded-sample-n")`.
