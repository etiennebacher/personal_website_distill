...
description: |
  Jarl is a new R linter built in Rust. It checks thousands of lines of R code
  in milliseconds and provides automatic fixes.
author:
  - name: Etienne Bacher
    url: {}
date: 2025-11-10
...
<video width="100%" height="100%" alt="Running `jarl check foo.R --fix`, where 'foo.R' is a file that contains two pieces of code that should be reported. This video shows that `any(is.na(x))` is replaced by `anyNA(x)`, and `apply(x, 1, mean)` is replaced by `rowMeans(x)`." controls>
  <source src="demos/automatic-fixes.webm" type="video/mp4">
</video>

<br>

Jarl is built on the shoulders of giants, in particular:

* `lintr`: this package provides dozens of rules for linting R code, and Jarl wouldn't exist without this package. Jarl currently supports 25 `lintr` rules.
* Air: this is a fast R formatter written in Rust, developed by Lionel Henry and Davis Vaughan, and released earlier this year. This is the technical foundation on which Jarl is built, since Air provides the infrastructure to parse and manipulate R code.
